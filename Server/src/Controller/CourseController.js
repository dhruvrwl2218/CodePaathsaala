...
export const RemoveCourse = async (req, res) => {
  ...
  const isCourseThere = await Course.findOne({ CourseId: CourseId });

  // console.log(isCourseThere)

  if (!isCourseThere)
    throw new ApiError(505, "no such Course is there ..", Error);

  const result = await Course.deleteOne({ CourseId: CourseId });

  if (!result) {
    throw new ApiError(400, "error while deleting the course", Error);
  }

  return res.status(200).json(new ApiResponse(200, "course has been deleted"));
};
...
export const UploadFiles = async (req, res) => {
  ...
  try {
    ...
    throw new ApiError("error occured while data uploading !", error);
  }
  ...
  try {
    ...
    console.log(error);
    throw ApiError(500, "unable to upload data in db", Error);
  }
  res
    .status(200)
    .json(
...
// }
  ...
  // }
  try {
  const Update = await Course.findByIdAndUpdate(_id,
    {$set : updatedfields},
    {new :true}
    )
  ...
  res.status(200)
  .json(new ApiResponse(200,Update,"Course Details updated successfully"))

  } catch (error) {
    return res.status(400)
    .json(new ApiResponse(500,"Updatation fails"))
  }
}
